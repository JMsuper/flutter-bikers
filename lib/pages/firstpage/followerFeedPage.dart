...
class FollowerFeedPage extends StatefulWidget {
  _FollowerFeedPageState createState() => _FollowerFeedPageState();
}

class _FollowerFeedPageState extends State<FollowerFeedPage> 
with TickerProviderStateMixin{
  ...
  _getProducts() async {
    if (_lastDocument == null) {
      FeedList feedList = await Feed.getFollowerFeed(user.uid, _pageSize);
      int listLength = feedList.feedList.length;
      _lastDocument = feedList.feedList[listLength - 1];
      return feedList;
    } else {
      FeedList feedList = await Feed.getFollowerFeedBeforeId(
